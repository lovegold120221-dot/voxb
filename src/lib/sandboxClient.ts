...
export async function createSandboxTask(type: string, label: string, prompt: string): Promise<{ taskId: string; task: ComputerTask }> {
  const res = await fetch(`${SANDBOX_URL}/api/tasks`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ type, label, userRequest: prompt }),
  });
