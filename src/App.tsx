...
- CRITICAL: When you execute a tool (like getting emails, creating a task, or executing a service), DO NOT STOP SPEAKING. You will immediately receive a background processing status. You MUST instantly continue the conversation while the real data loads in the background. Keep talking!
...
- Integrate the work into your conversational flow. Do not stay silent. Keep the conversation going seamlessly while you are waiting.
               ...
               try {
                 if (typeof session.sendClientContent === 'function') {
                   session.sendClientContent({
                     turns: "Start naturally like the conversation is already happening at a cafe. Do not introduce yourself. Do not mention your name. Do not offer help. Begin with a casual observation, small-talk thought, back-to-reality moment, or light current-topic style comment. Keep it calm and normal."
                   });
                 }
               } catch (e) {
        ...
        if (message.toolCall) {
           ...
           if (toolCalls && toolCalls.length > 0) {
               ...
               for (const call of toolCalls) {
                   ...
                   const executeTaskAsync = async (taskId: string, call: any) => {
                     try {
                       let result = null;
                       if (!googleToken) {
                       ...
                       // Report asynchronous result back to the AI
                         ...
                         if (typeof s.send === 'function') {
                           s.send({
                             clientContent: {
                               turns: [{
                                 role: 'user',
                                 parts: [{ text: `SYSTEM ASYNC RESULT for ${call.name}: ${JSON.stringify(result).substring(0, 3000)}` }]
                               }]
                             }
                           });
                         }
                         ...
                         if (typeof s.send === 'function') {
                           s.send({
                             clientContent: {
                               turns: [{
                                 role: 'user',
                                 parts: [{ text: `SYSTEM ASYNC ERROR for ${call.name}: ${String(err)}` }]
                               }]
                             }
                           });
                         }
                   ...
                   executeTaskAsync(taskId, call);

                   responses.push({
                       id: call.id,
                       name: call.name,
                       response: { result: { async_status: "Task started. You must IMMEDIATELY speak to the user to acknowledge you are working on it. Say something like 'Give me a second to pull that up...' and keep the conversation going." } }
                   });
               }
               if (responses.length > 0) {
                  sessionPromise.then((s: any) => {
                    // Using s.sendToolResponse since Gemini 2.0 uses it for tool responses sometimes
